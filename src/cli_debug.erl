...
trace_function(Module, Function, Opts) ->
    start_tracer(Opts),
...
message_flags(Opts, Acc) ->
    case proplists:get_value(send_only, Opts, false) of
        true -> [s|Acc];
        false ->
            case proplists:get_value(receive_only, Opts, false) of
                true -> [r|Acc];
                false -> [m|Acc]
            end
    end.
...
process_event_flags(Opts, Acc) ->
    case proplists:get_value(process_events, Opts, false) of
        true -> [p|Acc];
        false -> Acc
    end.
...
tracer(Opts) ->
    Pattern = pattern_match_spec(proplists:get_value(pattern, Opts)),
    Out = trace_device(proplists:get_value(file, Opts)),
...
match_spec(Opts) ->
    case proplists:get_value(no_return, Opts, false) of
	true ->
	    [{'_', [], []}];
	false ->
	    [{'_', [], [{return_trace}]}]
