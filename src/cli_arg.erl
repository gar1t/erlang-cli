...
-export([name/1, desc/1, arg_type/1, value_type/1, value_required/1,
         short_opt/1, long_opt/1]).

-record(arg, {key, desc, arg_type, value_type, value_required,
              short_opt, long_opt}).
...
new(Key, Desc, Opts) ->
    ...
    #arg{
       ...
       desc=Desc,
       arg_type=ArgType,
       value_type=value_type_from_opts(Opts),
       value_required=value_required_from_opts(Opts, ArgType),
       short_opt=ShortOpt,
       long_opt=LongOpt
      }.
...
value_required_from_opts(Opts, ArgType) ->
    Default = fun() -> default_value_required(Opts, ArgType) end,
    opt_val(required, Opts, Default).

default_value_required(Opts, ArgType) ->
    case arg_type_from_opts(Opts) of
        positional ->
            not opt_val(optional, Opts, false);
        option ->
            OptionDefault = fun() -> default_option_required(ArgType) end,
            opt_val(required, Opts, OptionDefault)
    end.

default_option_required(ArgType) ->
    ArgType /= bool.
...
value_required(#arg{value_required=Required}) -> Required.
